...
warnings.filterwarnings('ignore','<ipython-input-1-18ed73fecb34>:267: UserWarning: To copy construct from a tensor, it is recommended to use sourceTensor.clone().detach() or sourceTensor.clone().detach().requires_grad_(True), rather than torch.tensor(sourceTensor).y_target = torch.tensor(y_target, dtype=torch.long)')
...
PATH_DIR_LOGS_JSON = os.path.join(PATH_FILES,'logs_bayes_opt_semi_sup_induct2')
...
#label_smoothing = 0.1
#weights_loss = 'weighted' #Options: 'weighted_loss', 'non_weighted'
nearest_neighbors = 2
...
PATH_LOGS_JSON = os.path.join(PATH_DIR_LOGS_JSON,'logs_'+label+'_'+scheduler)

def bayes_opt_cv(lambda1,dropout,weights_loss,lr, num_layers,hidden_channels,use_lnorm):
    ...
    for i,(X_sel,X_labeled_train,X_unlabeled_train,X_val,X_test,labeled_train_inds, unlabeled_train_inds, val_inds, test_inds, _) in enumerate(cv_split(X,y[label],group,SELECT_SVC,train_size,val_size,data_subset = subset,cv_mode = cv_mode,subgraph_sampling = subgraph_sampling,K_train = K_train)):
        ...
        if arch == 'GCN_basic':
            ...
            model = GCN_basic_projection(num_layers = num_layers,activation = activation,dropout = dropout,
                hidden_channels=hidden_channels,input_size=num_features,output_size=num_classes,use_lnorm = use_lnorm,task='semi_sup').to(device)
        elif arch == 'GCN_test':
...
def bayes_opt_cv_for_discrete(lambda1,dropout,weights_loss,lr, num_layers,hidden_channels,use_lnorm):
...
if run_new is True:
    pbounds = {'lambda1':(0.001,0.5),'dropout':(0.1,0.7),
               'weights_loss':(0.0,1.0),'lr': (1e-6,0.01),
               'num_layers': (2.0, 6.0),'hidden_channels': (16.0, 128.0),
               'use_lnorm':(0.0,1.0)}

    "Bounded region of parameter space"
    #pbounds = {'lambda1': (0.001, 0.5),'label_smoothing': (0.0, 0.5),
    #        'dropout': (0.1, 0.7),'K_train_percentage': (0.1, 0.7)}
    optimizer = BayesianOptimization(
        f=bayes_opt_cv_for_discrete,
        pbounds=pbounds,
        random_state=1,
        verbose = 2,
    )
