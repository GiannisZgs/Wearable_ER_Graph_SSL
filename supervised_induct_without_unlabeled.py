...
PATH_RESULTS = os.path.join(PATH_FILES,'results_sup_induct_without_unlabeled_'+now_str)
PATH_LOGS = os.path.join(PATH_FILES,'logs_sup_induct_without_unlabeled_'+now_str)
...
if label == 'arousal_bin':
    K_train_percentage = 0.25 #Percentage of training participants se"lected to be labeled from training set
...
lambda2 = 0.0 #Regularization parameter for self-supervised loss
dropout = 0.5
use_lnorm = False
...
results_dir = os.path.join(PATH_RESULTS,cv_mode)
...
for i,(X_sel,X_labeled_train,X_unlabeled_train,X_val,X_test,labeled_train_inds, unlabeled_train_inds, val_inds, test_inds, _) in enumerate(cv_split(X,y[label],group,SELECT_SVC,train_size,val_size,data_subset = subset,cv_mode = cv_mode,subgraph_sampling = subgraph_sampling,K_train = K_train)):
    ...
    for epoch in range(1, epochs + 1):
        ...
        X_train = X_sel.iloc[Ls_train_inds]
        ...
        A_train = compute_adjacency(X_train,y_target[Ls_train_inds],construction_method,distance_metric,use_labels = use_labels,
            nearest_neighbors = nearest_neighbors,farthest_neighbors = farthest_neighbors,sup_mode = None,X_labels = X_train,X_no_labels = None)
                    # End the timer
        ...
        "Get labels"
        y_target_train = y_target[Ls_train_inds].clone().detach()
        ...
        if weights_loss == 'weighted':
            class_weights, class_distribution = get_class_weights(y_target_train)
        ...
        if epoch % plot_step == 0 and plot_times <= plot_number:
            if save_train_graph: 
                ...
                save_graph_dir = os.path.join(PATH_TRAINING_GRAPHS,cv_mode)
            ...
            y_plot = y_target_train.clone().detach()
        ...
        loss,_ = train(model, data, optimizer, criterion,device,lambda1 = lambda1)
    ...
    if save_test_graph: 
        ...
        save_graph_dir = os.path.join(PATH_TEST_GRAPHS,cv_mode)
        if not os.path.exists(save_graph_dir):
            os.makedirs(save_graph_dir)
