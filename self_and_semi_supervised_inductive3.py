...
augmentations = ['edge_removing']#,'denoising']#['node_masking','denoising','feature_masking','edge_perturbation'] #Options: denoising, shuffling, completion, mix
...
for augmentation in augmentations:
    ...
    log_dir = os.path.join(PATH_LOGS,cv_mode + '_' + augmentation)

    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    "Create results directory"
    results_dir = os.path.join(PATH_RESULTS,cv_mode + '_'+ augmentation)
    ...
    for i,(X_sel,X_labeled_train,X_unlabeled_train,X_val,X_test,labeled_train_inds, unlabeled_train_inds, val_inds, test_inds, _) in enumerate(cv_split(X,y[label],group,SELECT_SVC,train_size,val_size,data_subset = subset,cv_mode = cv_mode,subgraph_sampling = subgraph_sampling,K_train = K_train)):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % plot_step == 0 and plot_times <= plot_number:
                if save_train_graph: 
                    ...
                    save_graph_dir = os.path.join(PATH_TRAINING_GRAPHS,cv_mode + '_' + augmentation)
        ...
        if save_test_graph: 
            ...
            save_graph_dir = os.path.join(PATH_TEST_GRAPHS,cv_mode + '_' + augmentation)
            if not os.path.exists(save_graph_dir):
                os.makedirs(save_graph_dir)
