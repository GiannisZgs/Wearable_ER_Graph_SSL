...
PATH_RESULTS = os.path.join(PATH_FILES,'results_self_semi_sup_induct3_'+now_str)
PATH_LOGS = os.path.join(PATH_FILES,'logs_self_semi_sup_induct3_'+now_str)
...
K_train_percentage = 0.3 #Percentage of training participants se"lected to be labeled from training set
...
if K_train % 2 == 0:
    Ls_train = int(0.5*K_train)
    Us_train = int(0.5*K_train)
elif K_train % 2 == 1:
    Ls_train = int(np.floor(0.5*K_train)) #participants in training set with labels
    Us_train = int(np.ceil(0.5*K_train)) #participants in training set without labels
...
augmentation = 'node_masking' #Options: denoising, shuffling, completion, mix
if augmentation == 'denoising':
    graph_aug = graph_transforms.Denoising(p=feature_noise_percentage, mu=0.0, sigma=1.0)
elif augmentation == 'feature_masking':
    graph_aug = graph_transforms.Completion(p=masking_percentage,masking = 'feature')
elif augmentation == 'node_masking':
    graph_aug = graph_transforms.Completion(p=masking_percentage,masking = 'node')
elif augmentation == 'node_dropping':
    pass
    #graph_aug = A.NodeDropping(pn=node_dropping_prob)
elif augmentation == 'edge_perturbation':
    graph_aug = A.RandomChoice([A.EdgeAdding(pe=perturb_edge_prob),
                                A.EdgeRemoving(pe=perturb_edge_prob)],
                                num_choices=1)
